...
app.post("/register", async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: user.id, nome: user.nome, empresa: user.empresa, email: user.email },
...
app.put("/produtos/:id", auth, async (req, res) => {
  try {
    const { nome, sku, categoria, quantidade, minimo, preco, fornecedorId } = req.body;
    const dono = await prisma.produto.findFirst({
...
app.put("/fornecedores/:id", auth, async (req, res) => {
  try {
    const { nome, contato, email, telefone } = req.body;
    const dono = await prisma.fornecedor.findFirst({
      where: { id: Number(req.params.id), userId: req.user.id },
    });
    if (!dono) return res.status(404).json({ erro: "Fornecedor não encontrado" });
...
app.get("/relatorios/resumo", auth, async (req, res) => {
  try {
    ...
    const criticos = produtos.filter((p) => p.minimo > 0 && p.quantidade <= p.minimo).length;
...
app.post("/admin/cadastrar", adminAuth, async (req, res) => {
  try {
    const { nome, empresa, email, senha } = req.body;
    if (!nome || !empresa || !email || !senha)
      return res.status(400).json({ erro: "Preencha todos os campos" });
...
app.put("/funcionarios/:id", auth, async (req, res) => {
  try {
    const { nome, email, cargo } = req.body;
    const dono = await prisma.funcionario.findFirst({
      where: { id: Number(req.params.id), userId: req.user.id },
    });
    if (!dono) return res.status(404).json({ erro: "Funcionário não encontrado" });
...
app.get("/financeiro/resumo", auth, async (req, res) => {
  try {
    ...
    const produtos = await prisma.produto.findMany({ where: { userId } });
    const movimentacoes = await prisma.movimentacao.findMany({
      where: { userId, criadoEm: { gte: trintaDiasAtras } },
      include: { produto: true },
      orderBy: { criadoEm: "desc" },
      take: 500,
    });
...
app.put("/depositos/:id", auth, async (req, res) => {
  try {
    const { nome, descricao } = req.body;
    const dono = await prisma.deposito.findFirst({
      where: { id: Number(req.params.id), userId: req.user.id },
    });
    if (!dono) return res.status(404).json({ erro: "Depósito não encontrado" });
...
app.post("/depositos/:id/estoque", auth, async (req, res) => {
  try {
    ...
    const dono = await prisma.deposito.findFirst({
      where: { id: depositoId, userId: req.user.id },
    });
    if (!dono) return res.status(404).json({ erro: "Depósito não encontrado" });
    ...
    res.status(500).json({ erro: "Erro ao movimentar estoque no depósito" });
  }
});
...
app.patch("/pedidos/:id/status", auth, async (req, res) => {
  try {
    ...
    if (pedidoAtual.status === "recebido" && status === "recebido")
      return res.status(400).json({ erro: "Pedido já foi recebido" });
...
app.delete("/pedidos/:id", auth, async (req, res) => {
  try {
    const dono = await prisma.pedidoCompra.findFirst({
      where: { id: Number(req.params.id), userId: req.user.id },
    });
    if (!dono) return res.status(404).json({ erro: "Pedido não encontrado" });
...
app.get("/pedidos/criticos", auth, async (req, res) => {
  try {
    ...
    const criticos = produtos.filter(p => p.minimo > 0 && p.quantidade <= p.minimo);
    res.json(criticos);
  } catch (e) {
    res.status(500).json({ erro: "Erro ao buscar produtos críticos" });
  }
});
...
app.get("/relatorios/graficos", auth, async (req, res) => {
  try {
    ...
    const movimentacoes = await prisma.movimentacao.findMany({
      where: { userId, criadoEm: { gte: limite } },
      include: { produto: true },
    ...
    movimentacoes.forEach(m => {
      const key = new Date(m.criadoEm).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" });
      if (diasMap[key]) {
    ...
    res.status(500).json({ erro: "Erro ao buscar dados de gráficos" });
  }
});
